...
Copy-Item -Path './ni-grafana' -Destination $PluginRoot -Recurse -Force
Copy-Item -Path './custom.ini' -Destination "$GrafanaRoot/conf/" -Force
Copy-Item -Path './75_grafana.conf' -Destination "$FilesRoot/Web Server/conf/conf.d"
